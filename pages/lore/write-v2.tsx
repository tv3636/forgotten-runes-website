...
import { WizardConfiguration, WizardList } from "../../components/AddLore/WizardPicker";
import Spacer from "../../components/Spacer";
import { useEthers, useGasPrice } from "@usedapp/core";
import { formatUnits } from "ethers/lib/utils";
import { BigNumber } from "ethers";
import { BackgroundColorPickerField, NSFWField } from "../../components/AddLore/AddLoreFields";
import { useDebounce } from "react-use";
import { WriteButton } from "../../components/AddLore/AddLoreControls";
import { pinFileToIpfs, submitV2Lore, useExistingLoreData } from "../../components/AddLore/addLoreHelpers";
...
const WriteLore = ({}: {}) => {
  const [previewText, setPreviewText] = useState<string>(NEW_LORE_DEFAULT_MARKDOWN);
  const [editorText, setEditorText] = useState<string>(NEW_LORE_DEFAULT_MARKDOWN);
  ...
  return (
    <Flex flexDirection={"column"} pb={6} p={4} pr={4}>
      <Flex flexDirection="row" justifyContent={"space-between"} alignItems={"center"}>
        <Image src="/static/img/forgotten-runes-logo.png"
               width="200" height={"66"}
        />
        <Flex flexDirection={"row"} alignItems={"center"} pb={0} alignSelf={"center"}
        >

          {pickedToken && <img
            width={"80"}
            height={"80"}
            src={getTokenImageSrc(pickedToken.tokenId, pickedToken.tokenAddress)}
          />}


          <Spacer pl={2} />
          {pickedToken ? <h2>{pickedToken.name} #{pickedToken.tokenId} </h2> : <h2>Pick a character</h2>}
        </Flex>
        <div style={{ width: 200 }} />
      </Flex>
      ...
      {pickedToken && (
        <>
          <Flex flexDirection={"row"} height={"740px"} width={"100%"} style={{ position: "relative" }}>
            {submitting && <Flex flexDirection={"column"} justifyContent={"center"} alignItems={"center"} style={{
              position: "absolute",
              zIndex: 100,
              left: 0, top: 0, right: 0, bottom: 0, backgroundColor: "rgb(0,0,0,0.5)"
            }}>
          ...
          <Flex flexDirection={"row"} width={"100%"}>
            ...
            <Flex flex={1} pl={2} flexDirection={"row"} justifyContent={"flex-end"} alignItems={"center"}>
              ...
              <Spacer pl={3} />
              <WriteButton size="medium" onClick={async () => await submitV2Lore({
                nsfw: nsfw,
